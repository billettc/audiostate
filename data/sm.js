...
//     // handle manual calibration buttons click
//     const calibrationBtns = document.querySelectorAll('.calibration-btn');
//     calibrationBtns.forEach(function(btn) {
//         btn.onclick = function() {
//             // get manual calibration value
//             let calibrationValue = (isNaN(eval(localStorage.getItem('calibration-value'))) || localStorage.getItem('calibration-value') == null) ? 0 : localStorage.getItem('calibration-value');
//
//             // calibrate up is clicked
//             if (this.classList.contains('calibrate-up')) {
//                 calibrationValue = eval(calibrationValue) + 1;
//             } else {
//                 // calibrate down button is clicked
//                 calibrationValue =  eval(calibrationValue) - 1;
//             }
//
//             console.log(calibrationValue);
//             localStorage.setItem('calibration-value', calibrationValue);
//         }
//     });
//
...
function audioFromMic() {
    ...
    try {
        ...
        if (typeof navigator.mediaDevices['getUserMedia'] === 'undefined') {
            var j = {};
            j.googEchoCancellation = 'false';
            j.googAutoGainControl = 'false';
            j.googNoiseSuppression = 'false';
            j.googHighpassFilter = 'false';
            var l = {};
            l.mandatory = j;
            l.optional = [];
            var m = {};
            m.audio = l;
            navigator.getUserMedia(m, gotStream, didntGetStream);
        } else {
            var o = {};
            ...
            var r = {};
            r.audio = p;
            navigator.mediaDevices['getUserMedia'](r).then(gotStream).catch(didntGetStream);
        }
    } catch (s) {
        alert('getUserMedia threw exception :' + s);
    }
    // document.getElementById('readout').style['filter'] = 'none';
    startedMetering = !![];
}
